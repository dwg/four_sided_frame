...
You can also override just the base path of the images, keeping the names the same:
 <% four_sided_frame :base_path=>File.join(*%w(images my_frame)) do %>
 	Your content here
 <% end %>
This will load the images from public/images/my_frame/[top-right.png|bottom-left.png|full-frame.png].
...
* Since the outermost element is absolutely positioned it occupies no space in the page flow but as the top and left attributes are set to auto it is still positioned as it were in the flow. To position the frame I suggest using positive or negative values in the left and top margins.